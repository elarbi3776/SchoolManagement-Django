...
from formation import models as CMODEL
from formation import forms as CFORM
...
def customer_dashboard_view(request):
    dict={
        ...
        'available_course':CMODEL.Course.objects.all().count(),
        'applied_course':CMODEL.CourseRecord.objects.all().filter(customer=models.Customer.objects.get(user_id=request.user.id)).count(),
...
def apply_course_view(request):
    customer = models.Customer.objects.get(user_id=request.user.id)
    courses = CMODEL.Course.objects.all()
    return render(request,'customer/apply_course.html',{'courses':courses,'customer':customer})

def apply_view(request,pk):
    customer = models.Customer.objects.get(user_id=request.user.id)
    course = CMODEL.Course.objects.get(id=pk)
    courserecord = CMODEL.CourseRecord()
...
def history_view(request):
    customer = models.Customer.objects.get(user_id=request.user.id)
    courses = CMODEL.CourseRecord.objects.all().filter(customer=customer)
    return render(request,'customer/history.html',{'courses':courses,'customer':customer})
